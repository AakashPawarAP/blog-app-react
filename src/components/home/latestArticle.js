...
function LatestArticle(props) {
  ...
  useEffect(() => {
    const getData = () => {
      // .post("http://localhost:8000/api/v1/home-articles")
      // .then((res) => {
      axios
        .post(
      ...
      <div className="latest-article-container">
        <div className="card-container-lac">
          {art.map((val, ind) => {
            ...
            return (
              <div
                onClick={() => viewArticle(ind, val)}
                className="card"
                key={ind}
              >
                <img className="lac-img" src={val.img} alt="" />
                <div className="card-text">
                  <span className="card-heading">{val.heading}</span>
                  <p>{val.desc}</p>
                  <p>
                    <span className="travel-text">{val.type} </span>{" "}
                    <span className="travel-date">{val.date}</span>
                  </p>
                </div>
              </div>
            );
          })}
          <div className="load-more-articles" onClick={loadMore}>
            Load More &#8595;
          </div>
        </div>
